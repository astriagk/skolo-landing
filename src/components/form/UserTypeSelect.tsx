...
const options: { type: UserType; icon: string; title: string; description: string }[] = [
  {
    type: "parent",
    icon: "👨‍👩‍👧",
    title: "I'm a Parent",
...
export default function UserTypeSelect({ value, onChange, onSelect }: UserTypeSelectProps) {
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      {options.map((opt) => (
        <button
          key={opt.type}
          type="button"
          onClick={() => { onChange(opt.type); onSelect?.(opt.type); }}
          className={cn(
